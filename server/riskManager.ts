...
export class RiskManager {
  ...
  evaluateOrderRisk(
    ...
    if (activeTradeCount >= riskSettings.maxOpenTrades) {
      return {
        approved: false,
        rejectedReason: `Maximum open positions capacity reached (${activeTradeCount}/${riskSettings.maxOpenTrades}). Close an existing position before opening new trades.`,
        clampedPositionSize: 0,
        allocatedCapital: 0,
        stopLoss: 0,
        takeProfit: 0,
        ...
        riskAmount: 0,
        potentialProfit: 0,
        warnings: ['Capacity limit reached'],
      };
    }
